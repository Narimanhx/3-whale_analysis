...
NOTE
In the Resources folder of your starter code, you'll find CSV files for OTEX, L and SHOP. These are provided as backup in the event that the GOOGLEFINANCE function is not working properly.
...
Convert the S&P TSX 60 closing prices to daily returns. (5 points)
Join the whale returns, algorithmic returns, and the S&P TSX 60 returns into a single DataFrame with columns for each portfolio's individual returns. (8 points)
...
Determine which portfolios are riskier than the S&P TSX 60. (3 points)
Calculate the annualised standard deviation for each portfolio. (2 points)
Calculate and plot the rolling standard deviation for all portfolios using a 21-day window. (3 points)
Calculate and plot the correlation between each stock to determine which portfolios may mimic the S&P TSX 60. (3 points)
Choose one portfolio, then calculate and plot its beta as compared to the S&P TSX 60. (3 points)
...
Determine whether the algorithmic strategies outperform both the market (S&P TSX 60) and the whales portfolios. (8 points)
...
Add the portfolio returns to the DataFrame with the other portfolios, then analyse and compare all portfolios. (8 points)
...
Submission
Use the provided starter Jupyter Notebook to store the code for your data preparation, analysis, and visualisations. Put any analysis or answers to assignment questions in raw text (markdown) cells in the report.
